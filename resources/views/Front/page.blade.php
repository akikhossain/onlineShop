...
<script>
    $(document).ready(function() {
            $('#contactForm').submit(function(e) {
                ...
                $.ajax({
                    url: '{{ route('front.sentContactMail') }}',
                    type: 'post',
                    data: $(this).serialize(),
                    dataType: 'json',
                        ...
                        if (response.status == true) {
                            ...
                            if (errors.email) {
                                $("#email").addClass('is-invalid').siblings('p').addClass(
                                    'invalid-feedback').html(errors.email[0]);
                            } else {
                            ...
                            if (errors.message) {
                                $("#message").addClass('is-invalid').siblings('p').addClass(
                                    'invalid-feedback').html(errors.message[0]);
                            } else {
                                $("#message").removeClass('is-invalid').siblings('p')
                                    .removeClass('invalid-feedback').html("");
                            }
                            if (errors.subject) {
                                $("#subject").addClass('is-invalid').siblings('p').addClass(
                                    'invalid-feedback').html(errors.subject[0]);
                            } else {
